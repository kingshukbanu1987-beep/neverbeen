...
const countryPhoneData: readonly [string, string][] = [
  ...
  ['BO', '+591'], ['BQ', '+599'], ['BA', '+387'], ['BW', '+267'], ['BR', '+55'], ['IO', '+246'],
  ['VG', '+1'], ['BN', '+673'], ['BG', '+359'], ['BF', '+226'], ['BI', '+257'], ['CV', '+238'],
  ['KH', '+855'], ['CM', '+237'], ['CA', '+1'], ['KY', '+1'], ['CF', '+236'], ['TD', '+235'],
  ['CL', '+56'], ['CN', '+86'], ['CX', '+61'], ['CC', '+61'], ['CO', '+57'], ['KM', '+269'],
  ['CG', '+242'], ['CD', '+243'], ['CK', '+682'], ['CR', '+506'], ['CI', '+225'], ['HR', '+385'],
  ...
  ['TG', '+228'], ['TK', '+690'], ['TO', '+676'], ['TT', '+1'], ['TN', '+216'], ['TR', '+90'],
  ['TM', '+993'], ['TC', '+1'], ['TV', '+688'], ['UG', '+256'], ['UA', '+380'], ['AE', '+971'],
  ['GB', '+44'], ['US', '+1'], ['UY', '+598'], ['UZ', '+998'], ['VU', '+678'], ['VA', '+39'],
  ['VE', '+58'], ['VN', '+84'], ['VI', '+1'], ['WF', '+681'], ['EH', '+212'], ['YE', '+967'],
  ['ZM', '+260'], ['ZW', '+263'],
];

const countryPhoneOptions: readonly CountryPhoneOption[] = countryPhoneData.map(([isoCode, dialCode]) => ({
  isoCode,
  dialCode,
  flag: [...isoCode]
...
const countryOptions = countryPhoneOptions.map((country) => ({
  ...country,
  name: countryNameFormatter.of(country.isoCode) ?? country.isoCode,
}));

const birthDateValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
  if (!control.value) {
    return null;
  }
  ...
  return Number.isNaN(birthDate.getTime()) || birthDate > today ? { invalidBirthDate: true } : null;
};
...
export class Contact {
  ...
  protected readonly countries = countryPhoneOptions;
  protected readonly countryOptions = countryOptions;
  protected readonly destinations = contactDestinations;
  protected readonly selectedPackage = this.packageSelection.selectedPackage;

  protected readonly form = this.fb.nonNullable.group({
    name: ['', Validators.required],
    email: ['', [Validators.required, Validators.email, Validators.pattern(/^[^\s@]+@[^\s@]+\.[A-Za-z]{2,}$/)]],
    dateOfBirth: ['', [Validators.required, birthDateValidator]],
    gender: ['', Validators.required],
    country: ['', Validators.required],
    countryCode: ['+91', Validators.required],
    contactNumber: ['', [Validators.required, Validators.pattern(/^[0-9\s()-]{7,20}$/)]],
  ...
  submit(): void {
    ...
    const message = [
      ...
      `Contact Number: ${details.countryCode} ${details.contactNumber}`,
